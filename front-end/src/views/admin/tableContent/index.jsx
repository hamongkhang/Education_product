...
const TableContent = (props) => {
    ...
    const getTableOfContents = () =>{
        ...
        .then(data =>  {
            ...
            if(data.table_of_contents){
                if(param.id_course){
                    let table = data.table_of_contents.filter(item => item.course_id == param.id_course)
    ...
    const onChangeView = (e) =>{
        if(Number(e.target.value)){
            ...
            let tables1 = tables.filter(item => item.course_id == param.id_course)
    ...
    return (
        ...
        <section className="bg-blueGray-50">
            <h6 className="text-gray-700 text-xl font-bold mb-4">Mục lục</h6>
                           ...
                           <div className="relative w-full max-w-full flex-grow flex-1">
                               ...
                               <select name="type" id="type" className="text-13 px-3 py-1 outline-none border border-purple-800 focus:border-purple-900 rounded" 
                                onChange={(event) => onChangeView(event)}>
                                    ...
                                    {courses.map((item,index)=> <option key={index} value={item.id}>{item.name}</option>)}
                                </select>
                           </div>
                       {/* </div> */}
                       <div className="relative w-full max-w-full flex-grow flex-1 text-right">
                           ...
                           <div className={`absolute top-full right-0 ${classOption}`}>
                               <div className="py-2 bg-white shadow-lg text-13">
                                   <Link className="block w-full py-1 text-left px-2 hover:bg-gray-200" to={`/admin/table_content_add`} >Add</Link>
                               ...
                               tablesFilter.map((item,index)=>{
                                   ...
                                   <tr key={index}>
                                       ...
                                       <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">
                                           <Link to={`/admin/content/${item.id}`}>{item.name}</Link>
                                       </td>
                                       ...
                                       <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                           <div className="space-x-2">
                                               <Link to={`/admin/table_content/${item.id}/edit`} className="py-1 px-2 text-white rounded hover:opacity-80 bg-green-400 shadow-lg block md:inline-block">Edit</Link>
                                               <button className="py-1 px-2 text-white rounded hover:opacity-80 bg-red-500 shadow-lg block md:inline-block" onClick={()=>deleteTableContent(item.id)}>Delete</button>
                                               <Link to={`/admin/content/${item.id}`} className="py-1 px-2 text-white rounded hover:opacity-80 bg-blue-400 shadow-lg block md:inline-block"><i className="fas fa-arrow-right"></i></Link>
                                           </div>
                                       </td>
                                   </tr>
                                   )
