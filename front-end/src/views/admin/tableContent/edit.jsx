    ...
    const getOneTableOfContent = () =>{
        ...
        .then(data =>  {
            if(data.error){
                ...
                setTable(data.table_of_content);
            }
                ...
                <div className="flex flex-wrap">
                        ...
                        <div className="relative w-full mb-3">
                            ...
                            <label htmlFor={`toggle`} className="toggle-label">
                                <input type="checkbox" name="status" id={`toggle`} 
                                    onChange={(event) => onChangeHandle(event)}
                                    checked = {table.status === 'Active'?true:false}
                                    hidden />
                    ...
                    <div className="w-full lg:w-6/12 px-4">
                            <div className="relative w-full mb-3">
                                ...
                                <select name="course_id" id="type" className="border-0 px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" onChange={(event) => onChangeHandle(event)}>
                                   {
                                       courses.map((item,index)=>(
                                            <option key={index} value={item.id} selected = {item.id === table.course_id?true:false}>{item.name}</option>
                                       ))
                                   }
                    ...
                    <div className="w-full lg:w-6/12 px-4">
                        ...
                        <button type="button" onClick={()=>updateTableOfContent()} className="bg-indigo-600 text-white px-5 py-2 rounded hover:bg-indigo-700 hover:shadow-xl font-semibold duration-300">Cập nhật</button>
                    </div>
                </div>
                </form>
            </div>
