...
const DocumentTable = (props) => {
    ...
    const handleOptionDocument = () => {
        classOptionDocument === "hidden" ? setClassOptionDocument("block") : setClassOptionDocument("hidden")
    }
    ...
    const changeStatus = (id) =>{
        fetch(`http://localhost:8000/api/freeDocumentCategory/blockActiveDocumentCategory/${id}`, {
            method: "POST",
            headers: {"Authorization": `Bearer `+$token}
          })
        .then(response => response.json())
     ...
     const changeStatusDocument = (id) =>{
        fetch(`http://localhost:8000/api/freeDocument/blockActiveFreeDocument/${id}`, {
            method: "POST",
            headers: {"Authorization": `Bearer `+$token}
          })
        .then(response => response.json())
    ...
    const deleteDocumentCategory = (id) =>{
        ...
        fetch("http://localhost:8000/api/freeDocumentCategory/destroyFreeDocumentCategory/"+id, {
            method: "POST",
            body:_formData,
            headers: {"Authorization": `Bearer `+$token}
          })
    ...
    const deleteDocument = (id) =>{
        const _formData = new FormData();
        _formData.append("id",id)
        fetch("http://localhost:8000/api/freeDocument/destroyFreeDocument/"+id, {
            method: "POST",
            body:_formData,
            headers: {"Authorization": `Bearer `+$token}
          })
    ...
    return (
        ...
        <h6 className="text-gray-700 text-xl font-bold mb-4">Thông tin danh sách loại tài liệu</h6>
                ...
                <div className="relative w-full max-w-full flex-grow flex-1 text-right">
                    ...
                    <div className={`absolute top-full right-0 ${classOptionCategory}`}>
                        <div className="py-2 bg-white shadow-lg text-13">
                            <Link className="block w-full py-1 text-left px-2 hover:bg-gray-200" to={`other_document/addDocumentCategory`} >Add</Link>
                            ...
                            <tr key={index}>
                                ...
                                <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                    <div className="space-x-2">
                                        <Link to={`other_document/editDocumentCategory/${item.id}`} className="py-1 px-2 text-white rounded hover:opacity-80 bg-green-400 shadow-lg block md:inline-block">Edit</Link> 
                                        <button className="py-1 px-2 text-white rounded hover:opacity-80 bg-red-500 shadow-lg block md:inline-block" onClick={()=>onDeleteDocumentCategory(item.id)}>Delete</button>
                                    </div>
                                </td>
                            </tr>
                            )
    ...
    <section className="bg-blueGray-50">
        ...
        <div className="w-full">
            ...
            <div className="rounded-t mb-0 px-4 py-3 border-0">
                ...
                <select name="searchDocument" id="type" className="border-0 px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" onChange={(event) => onChangeHandle(event)}>
                ...
                <div className="relative w-full max-w-full flex-grow flex-1 text-right">
                    <button onClick={handleOptionDocument} className="bg-indigo-500 hover:bg-indigo-700 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                        <i className="far fa-ellipsis-v"></i>
                    </button>
                    <div className={`absolute top-full right-0 ${classOptionDocument}`}>
                        <div className="py-2 bg-white shadow-lg text-13">
                            <Link className="block w-full py-1 text-left px-2 hover:bg-gray-200" to={`other_document/addDocument`} >Add</Link>
                            ...
                            <button onClick={()=>importUser()} className={`w-full py-1 text-left px-2 hover:bg-gray-200 ${classOptionFile2}`} >Submit</button>
                            <button onClick={()=>ExportUser2()} className="w-full py-1 text-left px-2 hover:bg-gray-200">Export Excel</button>
                        </div>
                    </div>
                </div>
                </div>
                            ...
                            <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">
                                ...
                                <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                            <label htmlFor={`toggle${item.id}`} className="toggle-label">
                                                <input type="checkbox" name="" id={`toggle${item.id}`} 
                                                    defaultChecked = {item.status === 'Active'?true:false}
                                                    hidden onClick={()=>changeStatusDocument(item.id)}
                                                    />
                                                <div className="toggle-btn">
                                                    <div className="spinner"></div>
                                                </div>
                                ...
                                <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                    <div className="space-x-2">
                                        <Link to={`other_document/editDocument/${item.id}`} className="py-1 px-2 text-white rounded hover:opacity-80 bg-green-400 shadow-lg block md:inline-block">Edit</Link> 
                                        <button className="py-1 px-2 text-white rounded hover:opacity-80 bg-red-500 shadow-lg block md:inline-block" onClick={()=>onDeleteDocument(item.id)}>Delete</button>
                                    </div>
                                </td>
                            </tr>
                            )
