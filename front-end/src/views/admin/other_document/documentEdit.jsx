...
const EditDocument = () => {
    ...
    const updateDocumentAdmin = () => {
        ...
        fetch("http://localhost:8000/api/freeDocument/updateFreeDocument/"+documentEdit.id, {
            method: "POST",
            body:_formData,
    ...
    const onChangeHandle = (event) => {
        ...
        if(_type === "checkbox"){
            ...
            setDocumentEdit({...documentEdit,[_name]:event.target.files[0]});
        }
        else{
                ...
                <form>
                    ...
                    <div className="w-full lg:w-6/12 px-4">
                            <div className="relative w-full mb-3">
                                ...
                                <input type="text" onChange={(event) => onChangeHandle(event)}  value={documentEdit.name} name="name" required className="border-0 px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" onChange={(event) => onChangeHandle(event)}/>
                            ...
                            <div className="relative w-full mb-3">
                                ...
                                <select name="category_id" id="type" className="border-0 px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" onChange={(event) => onChangeHandle(event)}>
                                           {documentCategory.map((items)=>{
                                                 return(
                                                     <option value={items.id} selected = {documentEdit.category_id === items.id?true:false} >{items.name}</option>
                                                 )})}         
                                </select> 
                            </div>
                        </div>
                            ...
                            <div className="relative w-full mb-3">
                                ...
                                <label htmlFor={`toggle`} className="toggle-label">
                                    <input type="checkbox" name="status" id={`toggle`} 
                                        checked = {documentEdit.status === 'Active'?true:false}
                        ...
                        <div className="w-full lg:w-6/12 px-4">
                            <div className="relative w-full mb-3">
                                ...
                                <i>{typeof documentEdit.file==='string'?documentEdit.file:""}</i>
                    ...
                    <button type="button" onClick={()=>updateDocumentAdmin()} className="bg-indigo-600 text-white px-5 py-2 rounded hover:bg-indigo-700 hover:shadow-xl font-semibold duration-300">Cập nhập</button>
                    </form>
                </div>
                </div>
            </div>
