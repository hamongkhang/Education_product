            ...
            <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
                ...
                <div className="flex flex-wrap">
                    ...
                    <div className="w-full lg:w-6/12 px-4">
                        <div className="relative w-full mb-3">
                            ...
                            <label htmlFor={`toggle`} className="toggle-label">
                                <input type="checkbox" name="status" id={`toggle`} 
                                    onChange={(event) => onChangeHandle(event)}
                                    checked = {lesson.status === 'Active'?true:false}
                                    hidden />
                        ...
                        <div className="w-full lg:w-6/12 px-4">
                            <div className="relative w-full mb-3">
                                ...
                                <select name="content_id" id="type" className="border-0 px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" onChange={(event) => onChangeHandle(event)}>
                                    {
                                        contents.map((item,index)=>(
                                            <option key={index} value={item.id} selected = {item.id === lesson.content_id?true:false}>{item.name}</option>
                                        ))
                                    }
                        ...
                        <div className="w-full lg:w-6/12 px-4">
                            <div className="relative w-full mb-3">
                                ...
                                <input type="text" name="path" required className="border-0 px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" 
                                value={lesson.path?lesson.path:""}
                                onChange={(event) => onChangeHandle(event)}
                                />
                                <span className="text-red-500 text-sm">{error.name?error.name[0]:""}</span>
                            </div>
                        ...
                        <div className="w-full px-4">
                            <div className="relative w-full mb-3">
                                ...
                                <input type="file" id='avt' name="file" required onChange={(event) => onChangeHandle(event)}/>
                    ...
                    <div className="w-full lg:w-6/12 px-4">
                        ...
                        <button type="button" onClick={()=>updateLesson()} className="bg-indigo-600 text-white px-5 py-2 rounded hover:bg-indigo-700 hover:shadow-xl font-semibold duration-300">Cập nhật</button>
                    </div>
                </div>
                </form>
            </div>
