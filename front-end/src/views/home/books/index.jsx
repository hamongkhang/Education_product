        ...
        <>
            ...
            <div className="xl:w-4/5 xl:px-0 px-4 w-full relative left-1/2 transform -translate-x-1/2 mt-10">
                <div className="bg-purple-800 w-full h-12 md:h-16 rounded-md mb-10 flex items-center justify-between px-3 md:px-6" >
                    ...
                    <div>
                        <select name="bookSearch" onChange={(event) => addBookType(event)} id="" className="px-3 py-2 rounded-sm outline-none cursor-pointer">
                        <option name="bookSearch" value="allBook">Toàn bộ sách</option>
                        {bookType.map(
                         (item, index) => (
                            <option name="bookSearch" value={item.id}>{item.name}</option>
                         )
                         )}
                        </select>
                    </div>
