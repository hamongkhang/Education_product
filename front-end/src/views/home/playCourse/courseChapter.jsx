...
const CourseChapter = ({
    ...
    const handleChapterDetails = () => {
        if(chapter === "block") {
            ...
            //    )}
        <div className="w-full">
            <div className="flex items-center justify-between border-b cursor-pointer pb-2 p-4 bg-gray-100 hover:bg-gray-200 h-18" onClick={handleChapterDetails}>
                <div>
                    <div className="text-17 font-semibold">Phần {index+1}: {table.name}</div>
            ...
            <div className={chapter}>
                {
                    contentAlpha.map((item, index) => {
                        ...
                        <PlayCourseItem key={index} item={item} lesson={lesson} handleUrl={handleUrl}/>
                        )}})
                }
            </div>
       </div>
